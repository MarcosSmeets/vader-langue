...
struct Gen {
    ...
    has_fmt: bool,
    needs: Needs,
    /// `vader test` mode: inject `@vader_cov` at each user function's entry.
    cov: bool,
    /// names of the functions instrumented for coverage (the denominator).
    cov_fns: Vec<String>,
}
...
/// Compiles a program to native LLVM IR.
pub fn generate(program: &Program) -> Result<String, String> {
    generate_impl(program, None)
}

/// Compiles a program in `vader test` mode: emits each `test` block as a function,
/// injects per-function coverage, and synthesizes a `main` that runs the tests,
/// reports pass/fail + coverage, and exits 0 (ok) / 1 (failure) / 2 (coverage gate).
pub fn generate_tests(program: &Program, gate: bool, min_cov: f64) -> Result<String, String> {
    generate_impl(program, Some((gate, min_cov)))
}

fn generate_impl(program: &Program, test: Option<(bool, f64)>) -> Result<String, String> {
    let mut g = Gen {
        out: String::new(),
        globals: String::new(),
        tmp: 0,
    ...
    for item in &program.items {
        if let Item::Function(f) = item {
            if f.type_params.is_empty() {
                // in test mode the harness provides `main`; skip the user's.
                if test.is_some() && f.receiver.is_none() && f.name == "main" {
                    continue;
                }
    ...
    // `vader test`: each `test "name" { ... }` becomes a `void @__test_N()`, then a
    // synthetic `main` registers every function for coverage, runs the tests, and exits.
    if let Some((gate, min_cov)) = test {
        ...
        for item in &program.items {
            if let Item::Test(t) = item {
                let tf = Function {
                    ...
                    receiver: None,
                    name: format!("__test_{}", test_names.len()),
                    type_params: Vec::new(),
                    params: Vec::new(),
                    returns: Vec::new(),
                    body: t.body.clone(),
                };
                g.gen_function(&tf, None)?;
                test_names.push(t.name.clone());
            }
        }

        let cov_fns = g.cov_fns.clone();
        g.tmp = 0;
        g.label = 0;
        g.terminated = false;
        g.out.push_str("define i32 @main() {\nentry:\n");
        for fname in &cov_fns {
            let p = g.string_ptr(fname);
            g.emit(&format!("call void @vader_cov_register(i8* {})", p));
        }
        for (i, tname) in test_names.iter().enumerate() {
            let r = g.fresh();
            g.emit(&format!(
                "{} = call i32 @vader_test_run(void()* @__test_{})",
                r, i
            ));
            let p = g.string_ptr(tname);
            g.emit(&format!(
                "call void @vader_test_report_one(i8* {}, i32 {})",
                p, r
            ));
        }
        let code = g.fresh();
        g.emit(&format!(
            "{} = call i32 @vader_test_summary(i32 {}, double {:.1})",
            code,
            if gate { 1 } else { 0 },
            min_cov
        ));
        g.emit(&format!("ret i32 {}", code));
        g.out.push_str("}\n\n");
    }
    ...
    if test.is_some() {
        result.push_str(
            "declare void @vader_cov(i8*)\n\
             declare void @vader_cov_register(i8*)\n\
             declare i32 @vader_test_run(void()*)\n\
             declare void @vader_test_report_one(i8*, i32)\n\
             declare i32 @vader_test_summary(i32, double)\n",
        );
    }
    if g.needs.printf {
    ...
    if g.needs.http {
        result.push_str(
            "declare i8* @vader_http_listen(i64)\n\
             ...
             declare void @vader_http_json(i8*, i64, i8*)\n\
             declare void @vader_http_text(i8*, i64, i8*)\n\
...
impl Gen {
    ...
    fn gen_function(&mut self, f: &Function, name_override: Option<&str>) -> Result<(), String> {
        ...
        // `vader test` coverage: mark this function as hit at its entry. Skip `main`,
        // synthetic test blocks, and monomorphized generic instances.
        if self.cov && name_override.is_none() {
            let display = name.trim_matches('"').to_string();
            if display != "main" && !display.starts_with("__test_") {
                let p = self.string_ptr(&display);
                self.emit(&format!("call void @vader_cov(i8* {})", p));
                self.cov_fns.push(display);
            }
        }
    ...
    fn gen_http_call(
        ...
        let (en, ret, ptys): (&str, &str, &[&str]) = match name {
            ...
            "json" => ("vader_http_json", "void", &["i8*", "i64", "i8*"]),
            "text" => ("vader_http_text", "void", &["i8*", "i64", "i8*"]),
