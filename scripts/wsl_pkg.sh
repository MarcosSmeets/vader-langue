...
echo "=== sets up the 'greeter' lib as a local git repo ==="
...
echo "=== sets up the 'myapp' project that imports greeter ==="
mkdir -p /tmp/myapp
...
echo "=== vader check . (typecheck with the dep, without Go) ==="
"$BIN" check .
echo "=== vader run . (build via Go + runs) ==="
"$BIN" run .