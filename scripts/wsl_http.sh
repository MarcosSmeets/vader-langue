...
echo "=== compila runtimes http/json ==="
clang -c -O2 -Wall runtime/vader_http.c -o /tmp/h.o 2>/tmp/h.log && echo "http OK" || { echo http FALHOU; grep error: /tmp/h.log|head; }
clang -c -O2 -Wall runtime/vader_json.c -o /tmp/j.o 2>/tmp/j.log && echo "json OK" || { echo json FALHOU; grep error: /tmp/j.log|head; }

echo "=== json_demo (parse + build + encode) ==="
"$BIN" llvm examples/json_demo.vd 2>&1 | grep -vE 'emitted|compiled|linkando|compilando' | tail -3

echo "=== sobe o servidor HTTP em background ==="
...
echo "=== cliente HTTP da própria Vader (http.get) ==="
