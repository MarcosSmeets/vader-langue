...
echo "=== compiles the drivers (clang -c) ==="
for f in vader_pg vader_mysql; do
  if clang -c -O2 -Wall runtime/$f.c -o /tmp/$f.o 2>/tmp/$f.log; then echo "$f OK"; else echo "$f FAILED:"; grep 'error:' /tmp/$f.log | head; fi
done
if clang -c -O2 -Iruntime/sqlite runtime/vader_db.c -o /tmp/vdb.o 2>/tmp/vdb.log; then echo "vader_db OK"; else echo "vader_db FAILED:"; grep 'error:' /tmp/vdb.log | head; fi

echo "=== verifies the crypto (known vectors) ==="
...
echo   "expected      a9993e364706816aba3e25717850c26c9cd0d89d"
